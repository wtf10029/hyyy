...
class Product extends Base
{
    ...
    public function add()
    {
        if (request()->isPost()) {
            ...
            //添加文章内容
            $res = Db::name('product_en')->insert($data);
            //判断文章添加是否成功
            if ($res) {
    ...
    public function delAll()
    {
        ...
        foreach ($pid as $key => $val) {
            //查询产品表中的数据
            $data = Db::name('product_en')->where('id',$val)->find();
            //删除id
            unset($data['id']);

            //将数据存入回收站
            $res = Db::name('recycle_product_en')->insert($data);

            //删除产品表中的数据
            $re = Db::name('product_en')->delete($val);
        }
    ...
    {
        //查询产品表中的数据
        $data = Db::name('product_en')->where('id',$id)->find();
        //删除id
        unset($data['id']);
        ...
        if ($res) {
            //删除产品表中的数据
            $re = Db::name('product_en')->delete($id);
    ...
    public function edit($id)
    {
        if (request()->isPost()) {
            ...
            if (!empty($im)) {
                ...
                $data['thumb_name'] = $sname;
                //删除原缩略图
                $th = Db::name('product_en')->where('id',$id)->field('thumb_name')->find();
                $thumb_name = $th['thumb_name'];
                unlink("./uploads/thumb/".$thumb_name);
            }

            //更新文章信息
            $re = db('product_en')->where('id',$id)->update($data);
            if ($re >= 0) {
        ...
        //查询产品表
        $res = Db::name('product_en')->where('id',$id)->find();
    ...
    public function show()
    {
        ...
        //查询产品表
        $res = Db::name('product_en')->where('id',$id)->field('status')->find();
        $status = $res['status'];

        if ($status == '1') {
            $st['status'] = '2';
            $num = Db::name('product_en')->where('id',$id)->update($st);
            if ($num == '1') {
                return ['txt' => 'blank'];
            }
        } elseif($status == '2') {
