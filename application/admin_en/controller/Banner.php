...
class Banner extends Base
{
    ...
    public function add()
    {
        if (request()->isPost()) {
            ...
            //添加轮播内容
            $res = Db::name('banner_en')->insert($data);
            //判断轮播添加是否成功
            if ($res) {
        ...
        //查询轮播分类
        $classify = Db::name('category')->where('pid','6')->select();

        //查询最大轮播排序
        $maxRank = Db::name('banner_en')->max('rank');
    ...
    public function delAll()
    {
        ...
        foreach ($pid as $key => $val) {
            //查询轮播表中的数据
            $data = Db::name('banner_en')->where('id',$val)->find();
            //删除id
            unset($data['id']);

            //将数据存入回收站
            $res = Db::name('recycle_banner_en')->insert($data);

            //删除轮播表中的数据
            $re = Db::name('banner_en')->delete($val);
        }
    ...
    {
        //查询轮播表中的数据
        $data = Db::name('banner_en')->where('id',$id)->find();
        //删除id
        unset($data['id']);
        ...
        if ($res) {
            //删除轮播表中的数据
            $re = Db::name('banner_en')->delete($id);
    ...
    public function edit($id)
    {
        if (request()->isPost()) {
            ...
            if (!empty($im)) {
                ...
                $data['size'] = $width.'*'.$height;
                //删除原缩略图
                $th = Db::name('banner_en')->where('id',$id)->field('imagename')->find();
            ...
            //更新轮播信息
            $re = db('banner_en')->where('id',$id)->update($data);
            if ($re >= 0) {
        ...
        //查询轮播表
        $res = Db::name('banner_en')->where('id',$id)->find();
    ...
    public function show()
    {
        ...
        //查询轮播表
        $res = Db::name('banner_en')->where('id',$id)->field('status')->find();
        $status = $res['status'];

        if ($status == '1') {
            $st['status'] = '2';
            $num = Db::name('banner_en')->where('id',$id)->update($st);
            if ($num == '1') {
                return ['txt' => 'blank'];
            }
        } elseif($status == '2') {
