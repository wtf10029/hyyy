...
class Recycle extends Base
{
    ...
    {
        //查询回收站图片表
        $res = db('recycle_image_en')->select();

        //查询图片分类表
        $rec = db('category')->where('pid','8')->select();
        //重写分类数组
        $data = '';
    ...
    public function recoverImg()
    {
        ...
        if (request()->isPost()) {
            ...
            //查询回收站图片表
            $re = db('recycle_image_en')->where('id',$riid)->find();
            //删除id
            unset($re['id']);
            //图片数据插入图片表
            $imr = db('image_en')->insert($re);

            if ($imr) {
                //删除回收站图片信息
                db('recycle_image_en')->delete($riid);
        ...
        //查询图片表
        $res = db('recycle_image_en')->where('id',$id)->find();
        $cid = $res['classify'];
    ...
    {
        //得到图片路径
        $path = db('recycle_image_en')->where('id',$id)->field('name')->find();
        //删除数据表中的数据
        $res = db('recycle_image_en')->delete($id);
    ...
    {
        //查询回收站新闻表
        $res = Db::name('recycle_news_en')->select();
    ...
    {
        //查询回收站文章表
        $res = db('recycle_news_en')->where('id',$id)->find();

        //删除id
        unset($res['id']);

        //回收站数据插入新闻表
        $rec = db('news_en')->insert($res);

        //判断是否插入成功
        if ($rec) {
            //删除回收站新闻数据
            db('recycle_news_en')->delete($id);
    ...
    {
        //得到图片路径
        $name = db('recycle_news_en')->where('id',$id)->field('thumb_name')->find();
        //删除数据表中的数据
        $res = db('recycle_news_en')->delete($id);
    ...
    {
        //查询回收站轮播表
        $res = Db::name('recycle_banner_en')->select();
    ...
    {
        //查询回收站轮播表
        $res = db('recycle_banner_en')->where('id',$id)->find();

        //删除id
        unset($res['id']);

        //回收站数据插入轮播表
        $rec = db('banner_en')->insert($res);

        //判断是否插入成功
        if ($rec) {
            //删除回收站轮播数据
            db('recycle_banner_en')->delete($id);
    ...
    {
        //得到图片路径
        $name = db('recycle_banner_en')->where('id',$id)->field('imagename')->find();
        //删除数据表中的数据
        $res = db('recycle_banner_en')->delete($id);
    ...
    {
        //查询回收站产品表
        $res = Db::name('recycle_product_en')->select();
    ...
    {
        //查询回收站产品表
        $res = db('recycle_product_en')->where('id',$id)->find();

        //删除id
        unset($res['id']);

        //回收站数据插入产品表
        $rec = db('product_en')->insert($res);

        //判断是否插入成功
        if ($rec) {
            //删除回收站产品数据
            db('recycle_product_en')->delete($id);
    ...
    {
        //得到图片路径
        $name = db('recycle_product_en')->where('id',$id)->field('thumb_name')->find();
        //删除数据表中的数据
        $res = db('recycle_product_en')->delete($id);
