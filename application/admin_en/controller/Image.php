...
class Image extends Base
{
    ...
    {
        //查询图片表
        $res = db('image_en')->select();

        //查询图片分类表
        $re = db('category')->where('pid','8')->select();
    ...
    public function edit()
    {
        ...
        if (request()->isPost()) {
            ...
            $iid = input('post.id');
            //查询图片表,得到图片名
            $r = Db::name('image_en')->where('id',$iid)->field('name')->find();
            $old_name = $r['name'];
            ...
            //更新图片信息
            $up = db('image_en')->where('id',$iid)->update($data);
        ...
        //查询图片表
        $res = db('image_en')->where('id',$id)->find();
        $cid = $res['classify'];
    ...
    public function add()
    {
        ...
        //查询图片分类表
        $res = db('category')->where('pid','8')->select();
    ...
    public function del($id)
    {
        ...
        //将图片信息放入回收站表
        $rec = db('recycle_image_en')->insert($res);

        //判断是否添加成功
        if ($rec) {
            //删除图片表中的数据
            db('image_en')->delete($id);
    ...
    {
        //查询图片表
        $res = db('image_en')->where('classify',$cl)->select();
        ...
        //查询广告位分类
        $re = Db::name('category')->where('pid','8')->select();
